...
function generateRealityKeyPair() {
  ...
  // Xray-core has changed the exact wording of this output across versions
  // ("Private key:"/"Public key:" vs "PrivateKey:"/"Password:" etc), so
  // instead of matching specific label text, just grab every "label: value"
  // line and take the values in order - private key always prints first.
  const values = [...out.matchAll(/^[^\n:]*:\s*(\S+)\s*$/gm)].map(m => m[1]);
  if (values.length < 2) {
    throw new Error(`خروجی نامعتبر از xray x25519: ${out.trim().slice(0, 200)}`);
  }
  return { privateKey: values[0], publicKey: values[1] };
}
